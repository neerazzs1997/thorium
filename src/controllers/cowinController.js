 ...
 let getweather = async function(req,res){
    ...
    let resultcity = []
     ...
     for(let i = 0; i<cityname.length; i++){
        ...
        let cityobj = {city:cityname[i]}

        let option={
            
            method:"get",

            url:`http://api.openweathermap.org/data/2.5/weather?q=${cityname[i]}&appid=71d8776c734e526e9cbf2390153b803f`
        ...
        cityobj.temp = result.data.main.temp
           resultcity.push(cityobj)
     }

     const sorting = resultcity.sort((x,y)=>{
         return  x.temp - y.temp
     })
     res.status(200).send({msg:sorting,status:true})
